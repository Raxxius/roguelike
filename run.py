...
def init_dungeon(d_width, d_height):
    ...
    This will create a rectangle with all squares set to wall.
    """
    dungeon_map = {}
    for ycoord in range(d_height):
        for xcoord in range(d_width):
            # creates a dungeon tile with an x,y coord and sets 'Wall'.
...
def position_rooms(stdscr, rooms, dungeon_map, dungeon_width, dungeon_height, room_list):
    ...
    room_start = Room(xcoord, xcoord + rooms[0][1], ycoord, ycoord + rooms[0][2], "start")
    ...
    room_end = Room(xcoord, xcoord + rooms[0][1], ycoord, ycoord + rooms[0][2], "end")
