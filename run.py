...
def init_rooms(room_number):
    """
    This function creates a dictonary of rooms with the room number, length and height.
    The number of rooms added is dependant upon the dungeon_size() function.
    Rooms have a random size betweem 4 and 6 tiles. 
    Dungeon_width is called to allow for change of room size based on dungeon size.
    """
    rooms = {}
...
def position_rooms(rooms, dungeon_map, dungeon_width, dungeon_height):
    ...
    for room in rooms:
        ...
        if xcoord + rooms[room][1] > dungeon_width:
            xcoord = dungeon_width - rooms[room][1]
        if ycoord + rooms[room][2] > dungeon_height:
            ycoord = dungeon_height - rooms[room][2]
        for x in range(rooms[room][1]):
            xnew = xcoord + x
            for y in range(rooms[room][2]):
                ynew = ycoord + y
